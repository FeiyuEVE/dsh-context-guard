...
`./compaction` 的 `ArchiveCutEngine extends BasicCompactionEngine`，只覆写 `summarize()` 这一个受支持
的扩展点、**零模型调用**：每次压缩写一份无损 raw 归档 + 一份确定性事实摘要（digest），返回只带路径
的指针帧。格式契约见 `docs/digest-format.md`。
...
| `src/compaction.ts` | `ArchiveCutEngine`：覆写 `summarize()`；归档 + digest 落盘 + 跨次继承 + 指针帧 |
...
| `tests/` | vitest：8 个 spec + mock adapter / stub 引擎，无网络 |
