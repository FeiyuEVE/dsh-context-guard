...
- **布局要容器自适应，别写死列数（2026-09-12 手机端反馈）**：成对字段原先写死
  ...
  **容器宽度**决定。**别只靠 `@media` 按视口判断**：宿主壳完全可能「宽视口 + 窄内容区」（实测
  ...
  宽视口窄容器、桌面）。
- **贴底保存行（`position:sticky; bottom:0`）在宿主滚动窗口里会差一段 padding（2026-09-12 手机端反馈）**：
  本分节 15 项、实测高 ~2100px，宿主设置弹窗的滚动窗口只有 511px，保存按钮原本排在最后，手机用户
  要连滑三次才看得到（「移动端无法滚动、看不到保存」）。改成 sticky 贴底后仍有第二处坑：**行底边
  对齐的是滚动窗口的内容盒底边，不含宿主自己的 `padding-bottom`**（实测 `._7kpBWW_options` 是 24px），
  于是保存行下方留一条缝，正在滚过的下一组标题从缝里透出来。补一块同底色、向下溢出**该 padding 值**
  的 `::after`「围裙」即可，但**高度必须正好等于那段 padding**：高了会多出滚动余量（绝对定位后代也计入
  滚动溢出，先按 40px 做实测 `scrollHeight` 1990→2002、滚到底多出一截空白），矮了盖不住。
  验收脚本 `/home/feiyueve/tmp/cg-e2e/mobile-verify-043.cjs`（断言：初始即在视口内、滚到底不脱离、
  缝内 `elementFromPoint` 命中保存行自身、状态同容器、无横向溢出）。
...
- **确定性抽取会把「谈论错误」当成「发生错误」**：错误判据不能只看关键词。实测一条成功的
  ...
  「当前进展复述最后一条用户请求」——它已经在「主要意图」末项里，实测同一段 160 字符在一个
  478 token 的 digest 里出现两遍，**并因此把档位顶过下限**（去掉后 full 档 300→216 tokens）。
  加抽取规则时先问「这条信息在别的章节已经出现过吗」。
- **线上第一份 0.4.0 摘要暴露的四处误判（2026-09-12 重放实测）**：`epoch-8`（362 条消息 /
  193 次工具调用）的 `报错与修复` 6 条里 5 条是过程噪音、`主要意图` 首条不是人说的、
  `关键技术概念` 是扩展名。四处根因各不相同，修在 0.4.1：
  - **`isError: true` 不等于「值得交接的失败」**：`edit`/`write` 被 fs 观察策略拒绝
    （`file has not been read — read the file, then retry`）也算 error 块，于是同一步的
    「读一下就过」占了 5/6 条。现在 `PROCESS_NOISE` 先过一遍。
  - **数字前缀不是程序名**：`DIAGNOSTIC_LINE` 原是 `^[\w.+-]{1,32}:\s*\S`，而 `grep -n` 的输出行
    `38:### … 报错判据收严` 满足它；偏偏这条**变更日志标题**里含中文「报错」，又被 `ERROR_LINE`
    命中 —— 两个「都对」的判据合起来把标题抬成了报错。加 `(?![0-9]+:)`。
  - **`plugin notice` 的 `summary` 不一定是人话**：`tool-jobs` 的后台任务通知是 `role: 'user'` +
    `form: 'notice'`，`summary` 就是**跑过的命令行**，于是「notice 取 summary」这条规则把
    `bash cd … && sed …` 抬成 `主要意图` 首条。教训：给字段定抽取规则前要看**所有**生产者，
    不能只按自己的插件想。`isJobNotice()` 单列，没并进 `isInjectedContext`——后者还驱动
    `rawExcludeInjected`，而这类通知是一次性投递、不会重发。
    （顺带：`docs/digest-format.md` 曾写「form 为 …/notice 的消息」一律算注入，与代码不符，已改准。）
  - **概念要「程序名」，不是「命令行的第一个词」，更不是扩展名**：去掉扩展名后又冒出
    `for`（`for p in …`）与 `p=/home/…/@feiyueve/dsh-context-guard;`（变量赋值）。
    现在要求首词匹配 `^[a-z][a-z0-9._+-]*$` 且非 `SHELL_KEYWORD`/`COMMAND_NOISE`。
- **验证重放比构造用例更能发现问题（方法）**：把线上真实区间灌回抽取器，比手写 fixture 更早暴露
  `for` 和变量赋值这两种首词噪音 —— 它们是写用例时想不到的。重放要注意两件事，否则区间是**残的**：
  ① 工具结果在会话日志里是**独立记录类型**（`tool/result`），不是 `user/message` 的 content；
  ② `agent/inbox/spliced` 是**就地追加新消息**，被插入的文本不在目标消息的载荷里 ——
